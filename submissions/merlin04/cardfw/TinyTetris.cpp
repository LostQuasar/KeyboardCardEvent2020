...
/*
 * This has been modified to work with cardfw.
 */
...
TinyTetris::TinyTetris() {
  
}

void TinyTetris::OLEDCommand(byte command) {
...
void TinyTetris::OLEDData(byte data) {
  Wire.beginTransmission(OLED_ADDRESS);
...
void TinyTetris::Setup() {
  ...
  delay(20);
  OLEDCommand(0xC0); // Flip one way
  OLEDCommand(0xA0); // Flip other way
  delay(20);
  OLEDCommand(0x8D);
  delay(20);
  OLEDCommand(0x14);
...
void TinyTetris::fillTetrisArray(byte value) {
  for (char r = 0; r < 24; r++) {
    for (char c = 0; c < 14; c++) {
...
void TinyTetris::fillTetrisScreen(byte value) {
  for (int r = 1; r < 21; r++) {
    for (int c = 2; c < 12; c++) {
      tetrisScreen[c][r] = value;
    }
...
void TinyTetris::drawTetrisScreen() {
  for (byte r = 1; r < 21; r++) {
...
void TinyTetris::drawTetrisLine(byte x) {
...
void TinyTetris::loadPiece(byte pieceNumber, byte row, byte coloum, bool loadScreen) {
...
void TinyTetris::drawLandedPiece() {
...
void TinyTetris::movePieceRight() {
  oldPiece = currentPiece;
  currentPiece.Coloum = currentPiece.Coloum + 1;
  //check collision
  if (checkColloision())  {
...
bool TinyTetris::checkColloision() {
...
void TinyTetris::processCompletedLines() {
...
void TinyTetris::setScore(long score, bool blank)
...
void TinyTetris::setNextBlock(byte pieceNumber) {
