...
static SPosition Snake::tailPosition = {1, 7};
static SPosition Snake::headPosition = {1 + STARTING_SCORE + 1, 7};
static SPosition Snake::foodPosition = {32, 7};
...
Snake::Snake(Adafruit_SSD1306* _oled) {
  oled = _oled;
  oled->clearDisplay();
  oled->display();
}
...
void Snake::setup() {
  ...
  SPosition position = tailPosition;
...
void Snake::sendToGrid(SPosition position, bool value) {
  ...
  // oled->endData();
  for(int x_offset = 0; x_offset < 8; x_offset += 1) {
    oled->drawPixel((position.x + HORIZONTAL_GAP) * 2 + x_offset, position.y >> 2, ((displayRow >> x_offset) & 0x01));
  }
}

void Snake::addDeltaToPosition(SPosition &pos, Direction delta) {
  switch(delta) {
    case RIGHT:
      pos.x +=1;
      break;
...
bool Snake::checkForCollision(SPosition collisionPosition, bool includeHead) {
  SPosition snakePosition = tailPosition;
