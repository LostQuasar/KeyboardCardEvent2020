// File taken from Keebcard (https://github.com/rsheldiii/Keebcard/blob/master/Tetris.cpp), modified to support new screen, new GFX library, and new inputs
#ifndef ARDUINO_UNO
// Not compatible with Arduino Uno due to interrupts
...
#define BUTTON4_FLAG (!(buttonFlags & (1 << BUTTON4)))
#define BUTTON3_FLAG (!(buttonFlags & (1 << BUTTON3)))
#define BUTTON1_FLAG (!(buttonFlags & (1 << BUTTON1)))
...
#define SHOULD_MOVE_LEFT (BUTTON4_FLAG || (leftButton && frameTime - leftButton > REPEAT_DELAY))
#define SHOULD_ROTATE (BUTTON3_FLAG || (rightButton && frameTime - rightButton > REPEAT_DELAY))
#define SHOULD_MOVE_RIGHT (BUTTON1_FLAG || (upButton && frameTime - upButton > REPEAT_DELAY))
...
const Shape Tetris::shapes[uniqueShapes * 4] = {
...
Tetris::Tetris(Adafruit_SSD1306 *_oled)
{
    oled = _oled;
...
void Tetris::main()
{
    // Setup interrupts on pins A4, A6, A7
    // See https://github.com/SpenceKonde/megaTinyCore/blob/master/megaavr/extras/PinInterrupts.md
    PORTA.PIN4CTRL |= 0b00000101;
    PORTA.PIN6CTRL |= 0b00000101;
    //PORTA.PIN7CTRL |= 0b00000101;
...
void Tetris::end()
{
    ...
    // Disable interrupts (but keep the pullup bit)
...
void Tetris::addOrRemovePiece(bool add)
{
    for (uint8_t i = 0; i < PIXELS_IN_SHAPE; i++)
    {
        ...
        if (newPosition.y > BOARD_HEIGHT || (int8_t)newPosition.y < 0)
            continue;
        if (newPosition.x > BOARD_WIDTH || (int8_t)newPosition.x < 0)
            continue;
...
void Tetris::renderBoard(bool wholeScreen, bool addPiece)
{
    ...
    // Variable names assume screen is rotated
    for (uint8_t row = 0; row < 32; row++)
    {
        // 1 row on board = 4 rows on display, so draw the row 4 times
        for (uint8_t row_offset = 0; row_offset < 4; row_offset++)
        {
            oled->drawPixel(row*4 + row_offset, X_OFFSET - 1, true);
            oled->drawPixel(row*4 + row_offset, X_OFFSET + 49, true);
            for (uint8_t col = 0; col < 8; col++)
            {
                // 1 col on board = 6 cols on display, so draw the col 8 times
                for (uint8_t col_offset = 0; col_offset < 6; col_offset++)
                {
                    oled->drawPixel(row*4 + row_offset, col*6 + col_offset + X_OFFSET, ((board[row] >> col) & 0x01));
                }
            }
        }
    }
